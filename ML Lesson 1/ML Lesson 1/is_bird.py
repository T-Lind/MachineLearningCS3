...
#     get_items=get_image_files, 
# 
# To find all the inputs to our model, run the `get_image_files` function (which returns a list of all image files in
# a path).
# 
#     splitter=RandomSplitter(valid_pct=0.2, seed=42),
# 
...
# Now we're ready to train our model. The fastest widely used computer vision model is `resnet18`. You can train this
# in a few minutes, even on a CPU! (On a GPU, it generally takes under 10 seconds...) `fastai` comes with a helpful
# `fine_tune()` method which automatically uses best practices for fine tuning a pre-trained model, so we'll use that.
